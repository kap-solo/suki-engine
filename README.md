...
- `createCurrencyFormatter({ currency, language })` — Intl + social codes (`XGC` → GC, `XSC`/`XEC` → SC); optional `currencyDisplay` from authenticate when Intl fails; set from auth via `game.formatCurrency()`
