...
The template ships with Suki’s **Mobile L-first** layout, **`createBetUi`** betting controls, **`createGameMenu`** burger menu (Music + Sound effects volume sliders with mute icons), **`createGameAudio`**, and a **tap-to-continue preloader** (see main README). Add MP3/OGG files under `assets/audio/` and wire SFX in `js/audio.js` / `js/game.js`. Customize modal copy in `js/menu.js` and restyle via `betUi.css` / `gameMenu.css`.
